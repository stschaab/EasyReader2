// kpi_tolstoi_levels.js
// Misst die 5 Kern-KPIs + Satzstatistik fuer Original, B1 und A2
// von "Война и мир" (book-1.json) und gibt eine Vergleichstabelle aus.
//
// Methodik (identisch mit kpi_stats.js):
//   - Eigennamen (pymorphy3 Name-Tag) und nicht-in-Kelly-Wörter ignoriert.
//   - Frequenzliste = scripts/kelly-words.json (8535 Einträge).
//   - Wörter mit hohem Rang (z.B. Rang > 2000) bleiben DRIN (zeigen Level-Belastung).
//
// WICHTIG: B1 ist nur fuer 97 Chunks vorhanden, A2 fuer 1245. Damit der
// 3-Wege-Vergleich fair ist, wird nur auf Chunks gemessen, die ALLE drei
// Levels besitzen.
//
// Aufruf:  node scripts/kpi_tolstoi_levels.js

const fs = require("fs");
const path = require("path");
const { execFileSync } = require("child_process");

const b = require(path.join(__dirname, "..", "book-1.json"));
// Spalten von schwer -> leicht
const LEVELS = ["original", "B1", "A2"];
...
// --- Frequenzliste laden (Kelly) ---
const kelly = JSON.parse(fs.readFileSync(path.join(__dirname, "kelly-words.json"), "utf8"));
const ipmOf = {}, rankOf = {};
for (const w of kelly) { ipmOf[w.lemma.toLowerCase()] = w.ipm; rankOf[w.lemma.toLowerCase()] = w.rank; }
const byIpmDesc = [...kelly].sort((a, x) => x.ipm - a.ipm);
...
// --- Batch-Lemmatisierung (pymorphy3 nur einmal pro Level laden) ---
function lemmatizeBatch(texts) {
  return JSON.parse(execFileSync(
    "py", ["-3.13", path.join(__dirname, "lemmatize_ru_batch.py")],
...
function sentenceLengthsFromText(text) {
  // Marker wie (1261) und [...] entfernen, damit sie die Satzstatistik nicht verfälschen
  const clean = text.replace(/\[[^\]]*\]|\(\d+\)/g, " ");
  const sentences = clean.split(/[.!?]+/).map(s => s.trim()).filter(s => s.length > 0);
  return sentences.map(s => (s.match(/[А-Яа-яЁё]+/g) || []).length).filter(n => n > 0);
}
...
// --- Main: nur Chunks, die ALLE 3 Levels haben ---
const all = b.chunks.filter(c =>
  LEVELS.every(lvl => c[lvl] && c[lvl].trim().length > 0));
console.error(`Chunks mit allen ${LEVELS.length} Levels: ${all.length} von ${b.chunks.length} (B1 ist nur für die ersten 97 vorhanden).`);

const perLevel = {};
for (const lvl of LEVELS) {
  process.stderr.write(`  Lemmatisiere ${lvl}...`);
  const texts = all.map(c => c[lvl]);
...
console.log(`KPI-Vergleich · Война и мир (Tolstoi) · ${all.length} Chunks`);
console.log(`(nur Chunks mit allen Levels · Frequenzliste: Kelly)`);
...
const orig = perLevel.original, b1 = perLevel.B1, a2 = perLevel.A2;
const chk = (cond, txt) => (cond ? "✓ JA" : "✗ NEIN") + "  " + txt;
console.log(chk(a2.sentAvg <= 6,  `A2 Ø Wörter/Satz ≤ 6?    `) + `→ ${a2.sentAvg.toFixed(1)}`);
console.log(chk(b1.sentAvg <= 15, `B1 Ø Wörter/Satz ≤ 15?   `) + `→ ${b1.sentAvg.toFixed(1)}`);
...
console.log(`Satzverkürzung B1 vs. Orig:  ${((orig.sentAvg - b1.sentAvg) / orig.sentAvg * 100).toFixed(0)}%  (${orig.sentAvg.toFixed(1)} → ${b1.sentAvg.toFixed(1)})`);
console.log(`\nFrequenzliste = Kelly (russisch). Eigennamen (pymorphy3 Name-Tag) und`);
console.log(`nicht-in-Kelly-Wörter sind ignoriert, Wörter mit Rang > 2000 bleiben drin.`);

// Roh-Export
const out = { book: "Война и мир", chunksMeasured: all.length, levels: perLevel };
fs.writeFileSync(path.join(__dirname, "kpi_tolstoi_result.json"), JSON.stringify(out, null, 2));
console.log(`\nRohdaten: scripts/kpi_tolstoi_result.json`);
